...
struct plugin_data __stdcall CFunctionPluginPyScript::get_plugin_data() const
{
	...
	pluginData.name.FormatMessage(IDS_SCRIPT_PY_NAME, m_script_info.script.Left(m_script_info.script.ReverseFind(L'.')));
	pluginData.desc = m_script_info.desc.IsEmpty() ? pluginData.name : m_script_info.desc;

	return pluginData;
}

struct arg_count __stdcall CFunctionPluginPyScript::get_arg_count() const
{
...
const vector<update_data>& __stdcall CFunctionPluginPyScript::end(const vector<picture_data>& picture_data_list)
{
	//# plugin variables
	//
	...
	{
		ShowShellExecuteErrorMessage(handle_wnd, get_plugin_data().name, m_PythonExe, script, m_script_info.script);
	}
