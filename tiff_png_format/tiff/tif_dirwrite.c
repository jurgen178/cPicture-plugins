...
TIFFWriteDirectory(TIFF* tif)
{
	...
	if (!WriteOK(tif, &dircount, sizeof (dircount))) {
		TIFFErrorW(tif->tif_name, "Error writing directory count");
		goto bad;
	}
	if (!WriteOK(tif, data, dirsize)) {
		TIFFErrorW(tif->tif_name, "Error writing directory contents");
		goto bad;
	}
	if (!WriteOK(tif, &diroff, sizeof (diroff))) {
		TIFFErrorW(tif->tif_name, "Error writing directory link");
		goto bad;
	}
...
TIFFWriteData(TIFF* tif, TIFFDirEntry* dir, char* cp)
{
	...
	TIFFErrorW(tif->tif_name, "Error writing data for field \"%s\"",
...
#define	HDROFF(f)	((toff_t) &(((TIFFHeader*) 0)->f))
        ...
        {
            TIFFErrorW(tif->tif_name, "Error updating TIFF header");
            return (0);
        }
    }
    else
...
#define	HDROFF(f)	((toff_t) &(((TIFFHeader*) 0)->f))
		...
		if (!WriteOK(tif, &diroff, sizeof (diroff))) {
			TIFFErrorW(tif->tif_name, "Error writing TIFF header");
			return (0);
		}
		return (1);
	}
