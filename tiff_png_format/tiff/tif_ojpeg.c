...
_notSupported(register TIFF *tif)
  { const TIFFCodec *c = TIFFFindCODEC(tif->tif_dir.td_compression);
    ...
    TIFFErrorW(tif->tif_name,"%s compression is not supported",c->name);
    return 0;
  }
#endif /* never */
        ...
        case TIFFTAG_PHOTOMETRIC           :
          if (   (v32 = (*sp->vsetparent)(tif,tag,ap))
            ...
            if (td->td_refblackwhite = _TIFFmalloc(6*sizeof(float)))
              ...
              {
                TIFFErrorW(tif->tif_name,"Cannot define default reference black and white levels");
                v32 = 0;
              };
          return v32;
#       endif /* COLORIMETRY_SUPPORT */
    ...
    switch (tag)
     ...
     /* Validate the JPEG-process code. */
        ...
        case TIFFTAG_JPEGPROC              :
          switch (v32)
            {
              default: TIFFErrorW(tif->tif_name,"Unknown JPEG process");
                       return 0;
          ...
          if (v32)
            { uint32 *v;
              ...
              if (v32 > NUM_QUANT_TBLS)
                {
                  TIFFErrorW(tif->tif_name,"Too many quantization tables");
                  return 0;
                };
              i = 0;
              v = va_arg(ap,uint32 *);
       ...
       L: if (v32)
            { uint32 *v;
              ...
              if (v32 > NUM_HUFF_TBLS)
                {
                  TIFFErrorW(tif->tif_name,"Too many Huffman tables");
                  return 0;
                };
              v = va_arg(ap,uint32 *);
              i = 0;
               ...
               */
                  *to++ = 0;
                  ...
                  if (size > sizeof (*h)->huffval/sizeof *(*h)->huffval)
                    {
                      TIFFErrorW(tif->tif_name,"Huffman table too big");
                      return 0;
                    };
...
#   endif /* never */
     ...
     */
        if (!(tif->tif_base = _TIFFmalloc(tif->tif_size=TIFFGetFileSize(tif))))
          {
            TIFFErrorW(tif->tif_name,"Cannot allocate file buffer");
            return 0;
          };
        if (!SeekOK(tif,0) || !ReadOK(tif,tif->tif_base,tif->tif_size))
          {
            TIFFErrorW(tif->tif_name,"Cannot read file");
            return 0;
          }
      };
