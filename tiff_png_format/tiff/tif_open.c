...
int
_TIFFgetMode(const TCHAR* mode, const char* module)
{
	int m = -1;
...
)
{
	...
	if (m == -1)
		goto bad2;
	tif = (TIFF *)_TIFFmalloc(sizeof (TIFF) + (_tcslen(name) + 1) * sizeof(TCHAR));
	if (tif == NULL) {
		//TIFFError(module, "%s: Out of memory (TIFF structure)", name);
		goto bad2;
	}
	_TIFFmemset(tif, 0, sizeof (*tif));
	tif->tif_name = (TCHAR*)((char*)tif + sizeof (TIFF));
	...
	if (!ReadOK(tif, &tif->tif_header, sizeof (TIFFHeader))) {
		if (tif->tif_mode == O_RDONLY) {
			TIFFErrorW(name, "Cannot read TIFF header");
			goto bad;
		}
		/*
		...
		if (!WriteOK(tif, &tif->tif_header, sizeof (TIFFHeader))) {
			TIFFErrorW(name, "Error writing TIFF header");
			goto bad;
		}
		/*
	...
	if (tif->tif_header.tiff_magic != TIFF_BIGENDIAN &&
		...
		TIFFErrorW(name,  "Not a TIFF file, bad magic number %d (0x%x)",
