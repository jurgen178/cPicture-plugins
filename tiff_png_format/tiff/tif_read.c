...
TIFFSeek(TIFF* tif, uint32 row, tsample_t sample)
{
	...
	if (row >= td->td_imagelength) {	/* out of range */
		TIFFErrorW(tif->tif_name, "%lu: Row out of range, max %lu",
...
TIFFReadEncodedStrip(TIFF* tif, tstrip_t strip, tdata_t buf, tsize_t size)
{
	...
	if (strip >= td->td_nstrips) {
		TIFFErrorW(tif->tif_name, "%ld: Strip out of range, max %ld",
...
TIFFReadRawStrip(TIFF* tif, tstrip_t strip, tdata_t buf, tsize_t size)
{
	...
	if (!TIFFCheckRead(tif, 0))
		return ((tsize_t) -1);
	if (strip >= td->td_nstrips) {
		TIFFErrorW(tif->tif_name, "%lu: Strip out of range, max %lu",
...
TIFFReadEncodedTile(TIFF* tif, ttile_t tile, tdata_t buf, tsize_t size)
{
	...
	if (tile >= td->td_nstrips) {
		TIFFErrorW(tif->tif_name, "%ld: Tile out of range, max %ld",
...
TIFFReadRawTile(TIFF* tif, ttile_t tile, tdata_t buf, tsize_t size)
{
	...
	if (!TIFFCheckRead(tif, 1))
		return ((tsize_t) -1);
	if (tile >= td->td_nstrips) {
		TIFFErrorW(tif->tif_name, "%lu: Tile out of range, max %lu",
...
TIFFCheckRead(TIFF* tif, int tiles)
{
	if (tif->tif_mode == O_WRONLY) {
		TIFFErrorW(tif->tif_name, "File not open for reading");
		return (0);
	}
	if (tiles ^ isTiled(tif)) {
		TIFFErrorW(tif->tif_name, tiles ?
