...
TIFFRGBAImageBegin(TIFFRGBAImage* img, TIFF* tif, int stop, char emsg[1024])
{
	...
	if (!TIFFGetField(tif, TIFFTAG_COLORMAP,
	    ...
	    TIFFErrorW(TIFFFileName(tif), "Missing required \"Colormap\" tag");
	    return (0);
	}
	    ...
	    TIFFErrorW(TIFFFileName(tif), "Out of memory for colormap copy");
	    return (0);
        }
...
{
    if (img->get == NULL) {
	TIFFErrorW(TIFFFileName(img->tif), "No \"get\" routine setup");
	return (0);
    }
    if (img->put.any == NULL) {
	TIFFErrorW(TIFFFileName(img->tif),
    ...
    uint32 rwidth, uint32 rheight, uint32* raster, int stop)
{
	...
	TIFFErrorW(TIFFFileName(tif), emsg);
	ok = 0;
    }
    return (ok);
}
...
setorientation(TIFFRGBAImage* img, uint32 h)
{
	...
	TIFFWarningW(TIFFFileName(tif), "using bottom-left orientation");
    ...
    default:
	TIFFWarningW(TIFFFileName(tif), "using top-left orientation");
...
gtTileContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    if (buf == 0) {
	TIFFErrorW(TIFFFileName(tif), "No space for tile buffer");
	return (0);
    }
...
gtStripContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    if (buf == 0) {
	TIFFErrorW(TIFFFileName(tif), "No space for strip buffer");
	return (0);
    }
    y = setorientation(img, h);
    orientation = img->orientation;
...
makebwmap(TIFFRGBAImage* img)
{
    ...
    if (img->BWmap == NULL) {
	TIFFErrorW(TIFFFileName(img->tif), "No space for B&W mapping table");
	return (0);
    }
...
makecmap(TIFFRGBAImage* img)
{
	...
	256*sizeof (uint32 *)+(256*nsamples*sizeof(uint32)));
    if (img->PALmap == NULL) {
	TIFFErrorW(TIFFFileName(img->tif), "No space for Palette mapping table");
	return (0);
    }
...
buildMap(TIFFRGBAImage* img)
{
	...
	if (checkcmap(img) == 16)
	    ...
	    TIFFWarningW(TIFFFileName(img->tif), "Assuming 8-bit colormap");
	/*
